const API = {
  ...
  init() {
    const saved = localStorage.getItem('icsd_hanshin_api_queue');
    ...
    if (urlBackend === 'gas' || urlBackend === 'cf') {
      localStorage.setItem('icsd_hanshin_backend_override', urlBackend);
    }
    // 從 localStorage 讀取強制後端
    this._backendOverride = localStorage.getItem('icsd_hanshin_backend_override');
  ...
  switchBackend(target) {
    ...
    localStorage.setItem('icsd_hanshin_backend_override', target);
  ...
  clearBackendOverride() {
    localStorage.removeItem('icsd_hanshin_backend_override');
  ...
  _initLoadBalance() {
    ...
    var savedNode = localStorage.getItem('icsd_hanshin_api_node');
    if (savedNode !== null) {
    ...
    try { localStorage.setItem('icsd_hanshin_api_node', this._nodeIndex); } catch(e) {}
  },
  ...
  _failover(failedIndex) {
    ...
    for (var i = 1; i < activeCount; i++) {
      ...
      if (this._nodeHealth[next]) {
        ...
        try { localStorage.setItem('icsd_hanshin_api_node', next); } catch(e) {}
  ...
  saveQueue() { try { localStorage.setItem('icsd_hanshin_api_queue', JSON.stringify(this.queue)); } catch(e) {} },
  async processQueue() {
